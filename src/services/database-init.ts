...
export class DatabaseInitService {
  ...
  private async insertDefaultData(): Promise<void> {
    try {
      ...
      // Check and insert default whitelist tokens ONLY if table is empty
  ...
  async initializeDatabase(): Promise<boolean> {
    try {
      ...
      // Вставляем данные по умолчанию ТОЛЬКО если таблицы пустые
  ...
  async checkAndInitialize(): Promise<boolean> {
    try {
      ...
      // Если все таблицы существуют, вставляем данные по умолчанию ТОЛЬКО если таблицы пустые
      await this.insertDefaultData();
